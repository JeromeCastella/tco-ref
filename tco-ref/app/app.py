...
def reset_all_parameters():
    ...
    st.session_state['vehicle_care_annual'] = 150.0
    st.session_state['charging_infrastructure'] = 3040.0
    st.session_state['include_tires_x2'] = True
    st.session_state['apply_maint_7_over_6'] = True
    
    for key in list(st.session_state.keys()):
        if key.startswith('ice_') or key.startswith('bev_') or key.startswith('phev_'):
            del st.session_state[key]
    
    for prefix in ['recharge']:
        for suffix in ['_slider_a', '_slider_b', '_slider_c', '_prev_a', '_prev_b', '_prev_c']:
            key = f"{prefix}{suffix}"
...
if st.button("🔄 Réinitialiser", help="Réinitialise tous les paramètres aux valeurs par défaut"):
    reset_all_parameters()
    st.rerun()

prev_vehicle_class = st.session_state.get('prev_vehicle_class', None)
if prev_vehicle_class != vehicle_class:
    st.session_state['prev_vehicle_class'] = vehicle_class
    if prev_vehicle_class is not None:
        for key in list(st.session_state.keys()):
            if key.startswith('ice_') or key.startswith('bev_') or key.startswith('phev_'):
                del st.session_state[key]
...
spec_ice = make_spec(
    ...
    purchase_price=st.session_state.get('ice_purchase_price'),
    consumption_fuel=st.session_state.get('ice_consumption_fuel')
)
spec_bev = make_spec(
    Tech.BEV, vehicle_class, fuel_price, elec_home, elec_work, 
    elec_public, w_home, w_work, w_public,
    purchase_price=st.session_state.get('bev_purchase_price'),
    consumption_elec=st.session_state.get('bev_consumption_elec')
)
spec_phev = make_spec(
    Tech.PHEV, vehicle_class, fuel_price, elec_home, elec_work, 
    elec_public, w_home, w_work, w_public, phev_share_elec,
    purchase_price=st.session_state.get('phev_purchase_price'),
    consumption_fuel=st.session_state.get('phev_consumption_fuel'),
    consumption_elec=st.session_state.get('phev_consumption_elec')
)
...
with st.expander("⚙️ Coûts et consommations par technologie"):
    st.caption("Personnalisez les prix d'achat et consommations pour chaque technologie")
    
    st.markdown("**ICE (Moteur thermique)**")
    col1, col2 = st.columns(2)
    with col1:
        ice_defaults = get_default(Tech.ICE, vehicle_class)
        ice_purchase_price = st.number_input(
            "Prix d'achat ICE (CHF)", 
            min_value=0.0, 
            max_value=200000.0, 
            value=st.session_state.get('ice_purchase_price', float(ice_defaults['purchase_price'])),
            step=1000.0,
            key='ice_purchase_price'
        )
    with col2:
        ice_consumption_fuel = st.number_input(
            "Consommation ICE (L/100km)", 
            min_value=0.0, 
            max_value=30.0, 
            value=st.session_state.get('ice_consumption_fuel', float(ice_defaults['consumption_fuel_l_per_100'])),
            step=0.1,
            key='ice_consumption_fuel'
        )
    
    st.markdown("**BEV (Électrique)**")
    col1, col2 = st.columns(2)
    with col1:
        bev_defaults = get_default(Tech.BEV, vehicle_class)
        bev_purchase_price = st.number_input(
            "Prix d'achat BEV (CHF)", 
            min_value=0.0, 
            max_value=200000.0, 
            value=st.session_state.get('bev_purchase_price', float(bev_defaults['purchase_price'])),
            step=1000.0,
            key='bev_purchase_price'
        )
    with col2:
        bev_consumption_elec = st.number_input(
            "Consommation BEV (kWh/100km)", 
            min_value=0.0, 
            max_value=50.0, 
            value=st.session_state.get('bev_consumption_elec', float(bev_defaults['consumption_elec_kwh_per_100'])),
            step=0.1,
            key='bev_consumption_elec'
        )
    
    st.markdown("**PHEV (Hybride rechargeable)**")
    col1, col2, col3 = st.columns(3)
    with col1:
        phev_defaults = get_default(Tech.PHEV, vehicle_class)
        phev_purchase_price = st.number_input(
            "Prix d'achat PHEV (CHF)", 
            min_value=0.0, 
            max_value=200000.0, 
            value=st.session_state.get('phev_purchase_price', float(phev_defaults['purchase_price'])),
            step=1000.0,
            key='phev_purchase_price'
        )
    with col2:
        phev_consumption_fuel = st.number_input(
            "Consommation PHEV (L/100km)", 
            min_value=0.0, 
            max_value=30.0, 
            value=st.session_state.get('phev_consumption_fuel', float(phev_defaults['consumption_fuel_l_per_100'])),
            step=0.1,
            key='phev_consumption_fuel'
        )
    with col3:
        phev_consumption_elec = st.number_input(
            "Consommation PHEV (kWh/100km)", 
            min_value=0.0, 
            max_value=50.0, 
            value=st.session_state.get('phev_consumption_elec', float(phev_defaults['consumption_elec_kwh_per_100'])),
            step=0.1,
            key='phev_consumption_elec'
        )
...
with st.expander("⚙️ Plus de paramètres (globaux)"):
    ...
    if st.button("Appliquer les paramètres globaux"):
...
with st.expander("⚡ Prix énergie"):
    ...
    if st.button("Appliquer les prix énergie"):
...
with st.expander("🔌 Profil de recharge BEV/PHEV (Weighted Electricity Price)"):
    ...
    if st.button("Appliquer le profil de recharge"):
...
with st.expander("🔋 PHEV: Part électrique vs thermique"):
    ...
    if st.button("Appliquer le paramètre PHEV"):
